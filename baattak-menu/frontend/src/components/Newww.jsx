  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        ...
        setMenuItems(response.data.filter(item => item.category === 'New'));
      } catch (error) {
        setMenuItems([]);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">New</h1>
      </header>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {menuItems.length > 0 ? (
          ...
          <p className="text-center text-xl text-gray-600 col-span-full">No new items found.</p>
        )}
      </section>
    </div>
  );
